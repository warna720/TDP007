...
class Logic
    ...
    def initialize
        @langParser = Parser.new("Logic") do
            ...
            rule :assign do
                match('(', 'set', :var, :expr, ')') {|_, _, v, e| @vars[v] = e}
            end

            rule :expr do
                match('(', 'or', :expr, :expr, ')') {|_, _, a, b| a or b}
                match('(', 'and', :expr, :expr, ')') {|_, _, a, b| a and b}
                match('(', 'not', :expr, ')') {|_, _, a| not a}
                match(:term)
            end
            ...
            rule :var do
                match(/[a-zA-Z]+/) {|a| @vars[a] != nil ? @vars[a] : a }
            end
        end
    end
